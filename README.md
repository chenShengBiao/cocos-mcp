...
一个 Python MCP server，把 Cocos Creator 3.8 的项目操作（场景搭建、组件配置、资源导入、headless 构建、平台发布）封装成 **175 个结构化工具**，给 AI 客户端（Claude Code / Cursor / Claude Desktop / 自部署 OpenAI 兼容客户端）调用。2D 和基础 3D 小游戏都覆盖，UI token 系统 + 6 种游戏逻辑脚手架 + Playwright 闭环反馈让 AI **自己玩自己做的游戏**。
...
Claude 会自动调用 175+ MCP 工具完成全流程：初始化项目 → 写脚本（scaffold 常用模板）→ 构建场景 → headless 编译 → 浏览器预览 → **Playwright 自动点测 + 视觉回归** → 发布到 iOS/Android/微信小游戏。**全程不用打开 Cocos Creator 编辑器。**
...
| **🆕 UI 响应式助手** | 5 | `make_fullscreen` / `anchor_to_edge` / `center_in_parent` / `stack_vertically` / `stack_horizontally` —— 告别 Widget bitmask |
| **🆕 文本合成** | 1 | `add_styled_text_block` —— 标题 + 副标题 + 分割线 + 正文合成 |
...
| **🆕 闭环反馈** | 8 | Playwright 点击 / 按键 / 输入 / 拖拽 / 读 JS 状态 / 等待 / 序列批执行 / 视觉 diff —— AI 能"玩"自己的游戏 |
| **🆕 游戏脚手架** | 6 | `scaffold_input_abstraction` / `scaffold_score_system` / `scaffold_player_controller`（4 kinds）/ `scaffold_enemy_ai`（3 kinds）/ `scaffold_spawner`（2 kinds）/ `scaffold_game_loop` —— 生成 canonical .ts 直接挂节点 |
...
| **预制体** | 3 | 创建空 prefab / 实例化 prefab 到场景（带 fileId 唯一化）/ **`save_subtree_as_prefab`**（把已配好的场景子树一键提取成 prefab，含自包含校验） |
...
## 游戏逻辑脚手架：AI 不用手写 .ts 了

6 个 `cocos_scaffold_*` 工具生成 canonical .ts 启动模板，返回压缩 UUID，直接 `cocos_add_script` 挂到节点：

```python
# 输入抽象（键盘 + 方向键 + 触屏 → 统一 InputManager.I 单例）
input_r = cocos_scaffold_input_abstraction("/game")
# 分数系统（localStorage 持久化 + 可选 Label 自动渲染）
score_r = cocos_scaffold_score_system("/game")
# 游戏状态机（menu / play / over，每 state 两个 Inspector 回调）
loop_r = cocos_scaffold_game_loop("/game", states=["menu", "play", "over"])

# 挂 GameManager 节点
cocos_add_script(scene, gm_node, input_r["uuid_compressed"])
cocos_add_script(scene, gm_node, score_r["uuid_compressed"])
cocos_add_script(scene, gm_node, loop_r["uuid_compressed"])

# 玩家（4 种：platformer / topdown / flappy / click_only）
player_r = cocos_scaffold_player_controller("/game", kind="platformer")
# 敌人（3 种：patrol / chase / shoot）
enemy_r = cocos_scaffold_enemy_ai("/game", kind="shoot")
# 生成器（2 种：time 定时 / proximity 靠近触发）
spawner_r = cocos_scaffold_spawner("/game", kind="time")
```

所有模板的字段都走 `@property` 让 Inspector 可调；numeric 字段有 `{ tooltip: '...' }` 注解；InputManager 是单例，玩家脚本 null-check 它不存在也不 crash。脚手架**一次生成，代码跟着项目走**，以后改模板只改项目里那份 .ts。

## 闭环反馈：让 AI 自己玩自己做的游戏

`cocos_screenshot_preview` 让 AI 看到游戏画面；这套工具让 AI **操作 + 读状态**：

```python
cocos_start_preview("/game")

# 单个操作
cocos_click_preview("http://localhost:8080", x=480, y=320)
cocos_press_key_preview("http://localhost:8080", key="Space")

# 或一次 session 里跑整串（状态不重置）
cocos_run_preview_sequence("http://localhost:8080", actions=[
    {"kind": "click", "x": 480, "y": 320},          # 点"开始"
    {"kind": "wait", "ms": 500},                    # 等场景过渡
    {"kind": "key", "key": "Space"},                # 跳起
    {"kind": "wait", "ms": 200},
    {"kind": "read_state", "expression": "window.game.score"},  # 读分数
    {"kind": "screenshot"},                         # 截图（hex PNG）
])

# 视觉回归：纯 Pillow，不需要 Playwright
...
**Playwright 是可选依赖**（~200 MB chromium）。没装时 tool 返回明确的 install 命令 hint，不装本地也能继续用 screenshot diff。
...
│   ├── screenshot.py                 # Playwright 无头截图（lazy-import）
│   ├── interact.py                   # Playwright 点击/按键/拖拽/读 JS 状态/序列执行/视觉 diff
...
│   │   ├── ui_tokens.py              # 5 内置主题 + hex_to_rgba + preset 解析器
│   │   └── gen_image.py              # AI 素材生成 + 自动导入
│   ├── scaffolds/                    # 游戏逻辑 .ts 脚手架（6 个 canonical 模板）
│   │   ├── __init__.py               # re-export
│   │   ├── input.py                  # InputManager（WASD + 方向键 + 触屏统一单例）
│   │   ├── score.py                  # GameScore（当前/最高 + localStorage + Label auto-render）
│   │   ├── player.py                 # PlayerController 4 kinds（platformer/topdown/flappy/click）
│   │   ├── enemy.py                  # EnemyAI 3 kinds（patrol/chase/shoot）
│   │   ├── spawner.py                # Spawner 2 kinds（time/proximity）
│   │   └── game_loop.py              # GameLoop 状态机（每 state 两个 Inspector 回调）
...
│   │   ├── ui_patterns.py            # 6 个 UI 模式预设（dialog/menu/hud/card_grid/toast/spinner）
│   │   ├── ui_lint.py                # 8 条 UI 质量规则
│   │   ├── responsive.py             # 5 个响应式助手（fullscreen/anchor/center/stack_v/stack_h）
│   │   ├── styled_text.py            # 标题+副标题+分割线+正文合成
│   │   ├── animation_presets.py      # 6 种入场/反馈动画（fade/slide/scale/bounce/pulse/shake）
│   │   ├── media.py                  # audio / anim / particle / spine / DB / tiled / video / 场景全局 setters
│   │   ├── rendering.py              # 3 lights + MeshRenderer + SkinnedMeshRenderer
│   │   ├── prefab.py                 # create_prefab + instantiate_prefab + save_subtree_as_prefab
│   │   └── modules.py                # COMPONENT_REQUIRES_MODULE + audit_scene_modules
│   └── tools/                        # MCP 工具薄包装，按职责拆 10 模块
...
- **557 个 pytest 测试**，本地 < 10s 跑完。覆盖：UUID、meta、scene_builder、batch、8 个 Joint2D、**3D 物理（18 个）**、**3D 渲染（10 个）**、prefab create/instantiate/**save_subtree**、所有发布工具、AI 素材生成（HTTP mock）、chroma key、跨平台预览、**结构化错误分类器 + TS 诊断解析**、**TypedDict 契约漂移检测**、**场景读缓存命中 / mtime 失效 / LRU 淘汰**、**引擎模块 audit**、**构建后补丁 CRUD + 三种 kind + 路径注入防御 + drift 保护**、**Creator 安装 env var / PATH 探测**、**UI token 5 主题 + 8 条 lint 规则**、**Playwright 闭环反馈（mocked）+ 视觉 diff**、**6 个游戏脚手架（15 种 kind 组合）+ .ts 模板生成**。
